...
export const leadSchema = z.object({
  ...
  days: z.string().min(2, "Indica los días preferidos.").max(200).transform(clean).default(""),
  startDate: z.string().min(1, "Indica una fecha estimada.").max(30).transform(clean).default(""),
  transportNeed: z.enum(["Sí", "No", "No sé todavía"]).default("No sé todavía"),
